...
namespace SAPBO.JS.Business
{
    ...
    ///     Delete => Anulado
    /// </summary>
    public class FailureSeverityBusiness : SapB1GenericRepository<FailureSeverity>, IFailureSeverityBusiness
    {
        private const string _tableName = TableNames.FailureSeverity;
        private const string _cacheName = "FailureSeverities";

        private readonly IMemoryCache _memoryCache;

        public FailureSeverityBusiness(SapB1Context context, ISapB1AutoMapper<FailureSeverity> mapper, IMemoryCache memoryCache) : base(context, mapper, true)
        {
            _memoryCache = memoryCache;
        }

        private async Task<ICollection<FailureSeverity>> GetCache()
        {
            ICollection<FailureSeverity> objs = null;

            if (!_memoryCache.TryGetValue(_cacheName, out objs))
            {
                objs = await GetAllAsync("GP_WEB_APP_074");
        ...
        public async Task<ICollection<FailureSeverity>> GetAllAsync(Enums.StatusType statusType = Enums.StatusType.Todos)
        {
            ...
            //    ? GetAllAsync("GP_WEB_APP_074")
            //    : GetAllAsync("GP_WEB_APP_239", new List<dynamic> { (int)statusType });
        }

        public async Task<ICollection<FailureSeverity>> GetAllWithIdsAsync(IEnumerable<int> ids)
        {
            var objs = await GetCache();

            return objs.Where(x => ids.Any(y => y.Equals(x.Id))).ToList();

            //return GetAllAsync("GP_WEB_APP_304", new List<dynamic> { string.Join(",", ids) });
        }

        public async Task<FailureSeverity> GetAsync(int id)
        {
            var objs = await GetCache();

            return objs.FirstOrDefault(x => x.Id.Equals(id));

            //return GetAsync("GP_WEB_APP_073", new List<dynamic> { id });
        }

        public Task CreateAsync(FailureSeverity obj)
        {
        ...
        public async Task UpdateAsync(FailureSeverity obj)
        {
            //Get obj
        ...
        private static void CheckRules(FailureSeverity obj, Enums.ObjectAction objectAction, FailureSeverity currentObj = null)
        {
            switch (objectAction)
            {
        ...
        {
            var id = GetValue("GP_WEB_APP_072", "Id", null);
            if (id == null)
