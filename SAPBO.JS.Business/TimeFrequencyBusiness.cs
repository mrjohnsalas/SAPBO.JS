...
namespace SAPBO.JS.Business
{
    ...
    ///     Delete => Anulado
    /// </summary>
    public class TimeFrequencyBusiness : SapB1GenericRepository<TimeFrequency>, ITimeFrequencyBusiness
    {
        private const string _tableName = TableNames.TimeFrequency;
        private const string _cacheName = "TimeFrequencies";

        private readonly IMemoryCache _memoryCache;

        public TimeFrequencyBusiness(SapB1Context context, ISapB1AutoMapper<TimeFrequency> mapper, IMemoryCache memoryCache) : base(context, mapper, true)
        {
            _memoryCache = memoryCache;
        }

        private async Task<ICollection<TimeFrequency>> GetCache()
        {
            ICollection<TimeFrequency> objs = null;

            if (!_memoryCache.TryGetValue(_cacheName, out objs))
            {
                objs = await GetAllAsync("GP_WEB_APP_098");
        ...
        public async Task<ICollection<TimeFrequency>> GetAllAsync(Enums.StatusType statusType = Enums.StatusType.Todos)
        {
            ...
            //    ? GetAllAsync("GP_WEB_APP_098")
            //    : GetAllAsync("GP_WEB_APP_247", new List<dynamic> { (int)statusType });
        }

        public async Task<ICollection<TimeFrequency>> GetAllWithIdsAsync(IEnumerable<int> ids)
        {
            var objs = await GetCache();

            return objs.Where(x => ids.Any(y => y.Equals(x.Id))).ToList();

            //return GetAllAsync("GP_WEB_APP_310", new List<dynamic> { string.Join(",", ids) });
        }

        public async Task<TimeFrequency> GetAsync(int id)
        {
            var objs = await GetCache();

            return objs.FirstOrDefault(x => x.Id.Equals(id));

            //return GetAsync("GP_WEB_APP_097", new List<dynamic> { id });
        }

        public Task CreateAsync(TimeFrequency obj)
        {
        ...
        public async Task UpdateAsync(TimeFrequency obj)
        {
            //Get obj
        ...
        private static void CheckRules(TimeFrequency obj, Enums.ObjectAction objectAction, TimeFrequency currentObj = null)
        {
            switch (objectAction)
            {
        ...
        {
            var id = GetValue("GP_WEB_APP_096", "Id", null);
            if (id == null)
