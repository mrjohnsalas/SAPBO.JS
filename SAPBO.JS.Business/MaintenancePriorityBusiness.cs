...
namespace SAPBO.JS.Business
{
    ...
    ///     Delete => Anulado
    /// </summary>
    public class MaintenancePriorityBusiness : SapB1GenericRepository<MaintenancePriority>, IMaintenancePriorityBusiness
    {
        private const string _tableName = TableNames.MaintenancePriority;
        private const string _cacheName = "MaintenancePriorities";

        private readonly IMemoryCache _memoryCache;

        public MaintenancePriorityBusiness(SapB1Context context, ISapB1AutoMapper<MaintenancePriority> mapper, IMemoryCache memoryCache) : base(context, mapper, true)
        {
            _memoryCache = memoryCache;
        }

        private async Task<ICollection<MaintenancePriority>> GetCache()
        {
            ICollection<MaintenancePriority> objs = null;

            if (!_memoryCache.TryGetValue(_cacheName, out objs))
            {
                objs = await GetAllAsync("GP_WEB_APP_089");
        ...
        public async Task<ICollection<MaintenancePriority>> GetAllAsync(Enums.StatusType statusType = Enums.StatusType.Todos)
        {
            ...
            //    ? GetAllAsync("GP_WEB_APP_089")
            //    : GetAllAsync("GP_WEB_APP_248", new List<dynamic> { (int)statusType });
        }

        public async Task<ICollection<MaintenancePriority>> GetAllWithIdsAsync(IEnumerable<int> ids)
        {
            var objs = await GetCache();

            return objs.Where(x => ids.Any(y => y.Equals(x.Id))).ToList();

            //return GetAllAsync("GP_WEB_APP_307", new List<dynamic> { string.Join(",", ids) });
        }

        public async Task<MaintenancePriority> GetAsync(int id)
        {
            var objs = await GetCache();

            return objs.FirstOrDefault(x => x.Id.Equals(id));

            //return GetAsync("GP_WEB_APP_088", new List<dynamic> { id });
        }

        public Task CreateAsync(MaintenancePriority obj)
        {
        ...
        public async Task UpdateAsync(MaintenancePriority obj)
        {
            //Get obj
        ...
        private static void CheckRules(MaintenancePriority obj, Enums.ObjectAction objectAction, MaintenancePriority currentObj = null)
        {
            switch (objectAction)
            {
        ...
        {
            var id = GetValue("GP_WEB_APP_087", "Id", null);
            if (id == null)
