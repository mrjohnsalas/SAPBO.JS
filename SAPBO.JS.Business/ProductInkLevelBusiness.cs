...
{
    public class ProductInkLevelBusiness : SapB1GenericRepository<ProductInkLevel>, IProductInkLevelBusiness
    {
        private const string _tableName = TableNames.ProductInkLevel;
        private const string _cacheName = "ProductInkLevels";

        private readonly IMemoryCache _memoryCache;

        public ProductInkLevelBusiness(SapB1Context context, ISapB1AutoMapper<ProductInkLevel> mapper, IMemoryCache memoryCache) : base(context, mapper, true)
        {
            _memoryCache = memoryCache;
        }

        private async Task<ICollection<ProductInkLevel>> GetCache()
        {
            ICollection<ProductInkLevel> objs = null;

            if (!_memoryCache.TryGetValue(_cacheName, out objs))
            {
                objs = await GetAllAsync("GP_WEB_APP_168");
        ...
        public async Task<ICollection<ProductInkLevel>> GetAllAsync(Enums.StatusType statusType = Enums.StatusType.Todos)
        {
            ...
            //    ? GetAllAsync("GP_WEB_APP_168")
            //    : GetAllAsync("GP_WEB_APP_269", new List<dynamic> { (int)statusType });
        }

        public async Task<ICollection<ProductInkLevel>> GetAllWithIdsAsync(IEnumerable<int> ids)
        {
            var objs = await GetCache();

            return objs.Where(x => ids.Any(y => y.Equals(x.Id))).ToList();

            //return GetAllAsync("GP_WEB_APP_423", new List<dynamic> { string.Join(",", ids) });
        }

        public async Task<ProductInkLevel> GetAsync(int id)
        {
            var objs = await GetCache();

            return objs.FirstOrDefault(x => x.Id.Equals(id));

            //return GetAsync("GP_WEB_APP_167", new List<dynamic> { id });
        }

        public Task CreateAsync(ProductInkLevel obj)
        {
        ...
        public async Task UpdateAsync(ProductInkLevel obj)
        {
            //Get obj
            ...
            currentObj.ConsumoXje = obj.ConsumoXje;
        ...
        private static void CheckRules(ProductInkLevel obj, Enums.ObjectAction objectAction, ProductInkLevel currentObj = null)
        {
            switch (objectAction)
            {
        ...
        {
            var id = GetValue("GP_WEB_APP_166", "Id", null);
            if (id == null)
