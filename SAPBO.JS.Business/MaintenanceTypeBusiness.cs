...
namespace SAPBO.JS.Business
{
    ...
    ///     Delete => Anulado
    /// </summary>
    public class MaintenanceTypeBusiness : SapB1GenericRepository<MaintenanceType>, IMaintenanceTypeBusiness
    {
        private const string _tableName = TableNames.MaintenanceType;
        private const string _cacheName = "MaintenanceTypes";

        private readonly IMemoryCache _memoryCache;

        public MaintenanceTypeBusiness(SapB1Context context, ISapB1AutoMapper<MaintenanceType> mapper, IMemoryCache memoryCache) : base(context, mapper, true)
        {
            _memoryCache = memoryCache;
        }

        private async Task<ICollection<MaintenanceType>> GetCache()
        {
            ICollection<MaintenanceType> objs = null;

            if (!_memoryCache.TryGetValue(_cacheName, out objs))
            {
                objs = await GetAllAsync("GP_WEB_APP_086");
        ...
        public async Task<ICollection<MaintenanceType>> GetAllAsync(Enums.StatusType statusType = Enums.StatusType.Todos)
        {
            ...
            //    ? GetAllAsync("GP_WEB_APP_086")
            //    : GetAllAsync("GP_WEB_APP_246", new List<dynamic> { (int)statusType });
        }

        public async Task<ICollection<MaintenanceType>> GetAllWithIdsAsync(IEnumerable<int> ids)
        {
            var objs = await GetCache();

            return objs.Where(x => ids.Any(y => y.Equals(x.Id))).ToList();

            //return GetAllAsync("GP_WEB_APP_309", new List<dynamic> { string.Join(",", ids) });
        }

        public async Task<MaintenanceType> GetAsync(int id)
        {
            var objs = await GetCache();

            return objs.FirstOrDefault(x => x.Id.Equals(id));

            //return GetAsync("GP_WEB_APP_085", new List<dynamic> { id });
        }

        public Task CreateAsync(MaintenanceType obj)
        {
        ...
        public async Task UpdateAsync(MaintenanceType obj)
        {
            //Get obj
            ...
            currentObj.RequireFailure = obj.RequireFailure;
        ...
        private static void CheckRules(MaintenanceType obj, Enums.ObjectAction objectAction, MaintenanceType currentObj = null)
        {
            switch (objectAction)
            {
        ...
        {
            var id = GetValue("GP_WEB_APP_084", "Id", null);
            if (id == null)
